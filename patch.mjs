          ...
          {(['cph', 'aht', 'sla'] as const).map(tab => {
            ...
            return (
              <button
                ...
                className={\`pb-3 text-sm font-medium border-b-2 transition-all duration-200 \${ activeTab === tab ? 'border-brand-500 text-brand-600' : 'border-transparent text-surface-500 hover:text-surface-700 hover:border-surface-300' }\`}
              >
                {labels[tab]}
              </button>
            );
